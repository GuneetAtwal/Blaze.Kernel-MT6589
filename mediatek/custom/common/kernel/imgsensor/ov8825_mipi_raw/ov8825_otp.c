...
uint32_t R_gain1=0x800,B_gain1=0x800,G_gain1=0x800;
...
int update_otp_wb(void)
{
    ...
    if(current_otp.bg_ratio < BG_Ratio_typical)
    {
        if (current_otp.rg_ratio < RG_Ratio_typical)
        {
            ...
            G_gain = 0x800;
            B_gain = 0x800 * BG_Ratio_typical / current_otp.bg_ratio;
            R_gain = 0x800 * RG_Ratio_typical / current_otp.rg_ratio;
        }
        else
        {
            // current_otp.bg_ratio < BG_Ratio_typical &&
            // current_otp.rg_ratio >= RG_Ratio_typical
            R_gain = 0x800;
            G_gain = 0x800 * current_otp.rg_ratio / RG_Ratio_typical;
    ...
    else
    {
        if (current_otp.rg_ratio < RG_Ratio_typical)
        {
            ...
            // current_otp.rg_ratio < RG_Ratio_typical
            B_gain = 0x800;
            G_gain = 0x800 * current_otp.bg_ratio / BG_Ratio_typical;
        ...
        else
        {
            // current_otp.bg_ratio >= BG_Ratio_typical &&
            // current_otp.rg_ratio >= RG_Ratio_typical
            G_gain_B = 0x800 * current_otp.bg_ratio / BG_Ratio_typical;
            G_gain_R = 0x800 * current_otp.rg_ratio / RG_Ratio_typical;
